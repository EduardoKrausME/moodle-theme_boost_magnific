...
if (get_config('theme_boost_magnific', 'footer_type') != 0) {
    ...
    $html = "<a class='btn btn-info mt-1 mb-2' href='{$CFG->wwwroot}/theme/boost_magnific/_editor/?chave=footer&editlang=all'>{$text}</a>";
    ...
    if ($CFG->langmenu) {
        ...
        $text = get_string('editor_link_footer', 'theme_boost_magnific', $lang_name);
        $html = "<a class='btn btn-info mt-1 mb-2' href='{$CFG->wwwroot}/theme/boost_magnific/_editor/?chave=footer&editlang={$CFG->lang}'>{$text}</a>";
        ...
        foreach ($list_of_translations as $lang_key => $lang_name) {
            ...
            $text = get_string('editor_link_footer', 'theme_boost_magnific', $lang_name);
            $html = "<a class='btn btn-info mt-1' href='{$CFG->wwwroot}/theme/boost_magnific/_editor/?chave=footer&editlang={$lang_key}'>{$text}</a>";
            if (!isset(get_config("theme_boost_magnific", "footer_htmleditor_{$lang_key}")[3]))
                $html = "{$html} <strong class='alert-warning'>{$empty_text}</strong>";
            $description .= "{$html}<br>";
        }
    }
