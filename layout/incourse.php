...
$bodyattributes = $OUTPUT->body_attributes(["uses-drawers", theme_boost_magnific_get_body_class()]);
...
$templatedata = [
    ...
    "courseindex" => \theme_boost_magnific\template\course::courseindex($COURSE->id),
    "show_image_top_course" => \theme_boost_magnific\template\course::show_image_top_course($COURSE),
...
if (!$courseupdate && strpos($_SERVER["REQUEST_URI"], "/scorm/player.php") > 1) {
    ...
    if ($COURSE->format == "topics" || $COURSE->format == "weeks") {
        echo $OUTPUT->render_from_template("theme_boost_magnific/incourse", $templatedata);
    } else {
