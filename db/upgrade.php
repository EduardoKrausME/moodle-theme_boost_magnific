...
function xmldb_theme_boost_magnific_upgrade($oldversion) {
    ...
    if ($oldversion < 2024062001) {
        ...
        for ($i = 1; $i <= 4; $i++) {
            ...
            $file = $fs->create_file_from_pathname($filerecord,
                "{$CFG->dirroot}/theme/boost_magnific/pix/blocks/{$block['icon']}.svg");
        ...
        upgrade_plugin_savepoint(true, 2024062001, "theme", "boost_magnific");
    }

    if ($oldversion < 2024070900) {

        for ($i = 1; $i <= 4; $i++) {
            $url = get_config("theme_boost_magnific", "mycourses_url_{$i}");
            $url = str_replace("{{{config.wwwroot}}}", $CFG->wwwroot, $url);
            set_config("mycourses_url_{$i}", $url, "theme_boost_magnific");
        }

        upgrade_plugin_savepoint(true, 2024070900, "theme", "boost_magnific");
    }
    ...
    if ($oldversion < 2024071000) {
        if ($CFG->theme == "degrade") {
            ...
            $cor = get_config("theme_boost_magnific", "background_color");
            $newcolor = $cores[$cor];
            set_config("background_color", $newcolor, "theme_boost_magnific");
        }

        upgrade_plugin_savepoint(true, 2024071000, "theme", "boost_magnific");
    }

    if ($oldversion < 2025020600) {
        set_config("pagefonts", null, "theme_boost_magnific");
        ...
        upgrade_plugin_savepoint(true, 2025020600, "theme", "boost_magnific");
    }
