...
if (file_exists("{$CFG->dirroot}/theme/boost_magnific/config.php")) { // phpcs:disable Generic.CodeAnalysis.EmptyStatement.DetectedIf
    // The theme exists in standard location - ok.
} elseif (!empty($CFG->themedir) && file_exists("{$CFG->themedir}/boost_magnific/config.php")) { // phpcs:disable Generic.CodeAnalysis.EmptyStatement.DetectedIf
...
$lock = $lockfactory->get_lock("boost_magnific", rand(90, 120));
...
function theme_styles_get_filename($rev, $type, $themesubrev = 0, $usesvg = true) {
    ...
    $cssbasedir = "{$CFG->localcachedir}/theme/{$rev}/boost_magnific/css";
