...
function boost_magnific_get_accessibility_courseid() {
...
function boost_magnific_get_accessibility_cmid() {
    var body = document.body;
    if (!body || !body.className) {
        return 0;
    }
...
function boost_magnific_get_accessibility_active_items_list() {
    var state = window.boost_magnific_toolboxAppstate || {};
    var activeItems = [];
...
function boost_magnific_get_accessibility_active_items() {
    return boost_magnific_get_accessibility_active_items_list().join(', ');
}

function boost_magnific_log_accessibility(action, item, status, activeItemsOverride, stateOverride) {
    if (!window.M || !M.cfg || !M.cfg.wwwroot || !M.cfg.sesskey || typeof fetch === 'undefined') {
        return;
    }

    var state = stateOverride || window.boost_magnific_toolboxAppstate || {};
    var params = new URLSearchParams();
    params.append('sesskey', M.cfg.sesskey);
    params.append('action', action || '');
    params.append('item', item || '');
    params.append('status', status || '');
    params.append('courseid', boost_magnific_get_accessibility_courseid());
    params.append('cmid', boost_magnific_get_accessibility_cmid());
    params.append('pageurl', window.location.href);
    params.append('activeitems', typeof activeItemsOverride === 'string' ? activeItemsOverride : boost_magnific_get_accessibility_active_items());
    params.append('statejson', JSON.stringify(state));

    fetch(M.cfg.wwwroot + '/theme/boost_magnific/accessibility-ajax.php', {
        method: 'POST',
...
function boost_magnific_track_vlibras_access_button() {
    if (window.boost_magnificVlibraAccessButtonTracked) {
        return;
    }

    window.boost_magnificVlibraAccessButtonTracked = true;
    document.addEventListener('click', function (event) {
        ...
        if (target.closest('div[vw-access-button]')) {
            boost_magnific_log_accessibility('vlibras_tab_click', 'vw-access-button', 'clicked');
        }
    }, true);
}

function boost_magnific_track_toolbar_accessibility() {
    if (window.boost_magnificToolbarAccessibilityTracked) {
        return;
    }

    window.boost_magnificToolbarAccessibilityTracked = true;
    ...
    document.addEventListener('click', function (event) {
        ...
        if (toolbarButton) {
            window.boost_magnificToolbarBeforeItems = boost_magnific_get_accessibility_active_items_list();
        }

        var resetButton = target.closest('#mic-toolbox-disable-buttons-reset-all');
        if (resetButton) {
            boost_magnific_log_accessibility('toolbar_reset', resetButton.id, 'disabled', '', {
                bodyClassList: {},
                fontSize: 1,
                imagesTitle: false,
                keyboardRoot: false,
    ...
    document.addEventListener('click', function (event) {
        ...
        if (openButton) {
            boost_magnific_log_accessibility('toolbar_open', openButton.id, 'opened');
            return;
        }
        ...
        var beforeItems = window.boost_magnificToolbarBeforeItems || boost_magnific_get_accessibility_active_items_list();

        setTimeout(function () {
            var afterItems = boost_magnific_get_accessibility_active_items_list();
            var status = afterItems.indexOf(itemid) === -1 ? 'disabled' : 'enabled';

            if (itemid === 'mic-toolbox-fonts-up' || itemid === 'mic-toolbox-fonts-down') {
                status = Number((window.boost_magnific_toolboxAppstate || {}).fontSize || 1) > 1 ? 'enabled' : 'disabled';
            }

            boost_magnific_log_accessibility('toolbar_item', itemid, status);

            beforeItems.forEach(function (previousItem) {
                if (previousItem !== itemid && afterItems.indexOf(previousItem) === -1) {
                    boost_magnific_log_accessibility('toolbar_item', previousItem, 'disabled');
                }
            });
        }, 0);
    }, false);
...
function boost_magnific_AccessTool(html) {
    this.init = {
        link: "",
        contact: "",
...
boost_magnific_AccessTool.prototype.initialApp = function () {
    ...
    if (window.boost_magnific_toolboxAppstate.bodyClassList) {

        for (var bodyClass in window.boost_magnific_toolboxAppstate.bodyClassList) {
            var initBodyClassList = window.boost_magnific_toolboxAppstate.bodyClassList[bodyClass];
    ...
    // FONT SIZE INIT
    if (window.boost_magnific_toolboxAppstate.fontSize > 1) {
        this.initFontsChange();
    }

    // SET IMAGES TITLES
    if (window.boost_magnific_toolboxAppstate.imagesTitle) {
        this.imagesAddTitles();
    }

    // SET KEBOARD ROOTING
    if (window.boost_magnific_toolboxAppstate.keyboardRoot) {
...
boost_magnific_AccessTool.prototype.initialApp_iframe = function (iframe) {
    setInterval(function () {
        var appstate = JSON.parse(localStorage.getItem('boost_magnific_ACCESSTOOL')) || {
            bodyClassList: false
        };
...
boost_magnific_AccessTool.prototype.buildToolBox = function (html) {
...
// CONTRAST FUNCTION
boost_magnific_AccessTool.prototype.contrastChange = function (event) {
    ...
    if (document.body.classList.contains(this.id)) {
        ...
        delete window.boost_magnific_toolboxAppstate.bodyClassList[this.id];
    } else {
        ...
        for (var i = 0; i < buttons.length; i++) {
            ...
            delete window.boost_magnific_toolboxAppstate.bodyClassList[buttons[i].id];
        }
        this.classList.add('vi-enabled');
        document.body.classList.add(this.id);

        window.boost_magnific_toolboxAppstate.bodyClassList[this.id] = this.id;
    }
    boost_magnific_AccessTool.prototype.updateState();
};

// CURSOR CHANGE
boost_magnific_AccessTool.prototype.cursorChange = function (event) {
    event.preventDefault();
...
boost_magnific_AccessTool.prototype.onceButtonChange = function (event) {
    event.preventDefault();

    if (this.id === 'mic-toolbox-disable-buttons-keyboard') {
        window.boost_magnific_toolboxAppstate.keyboardRoot = !window.boost_magnific_toolboxAppstate.keyboardRoot;
        boost_magnific_AccessTool.prototype.keyboardRootEnable();
    }

    if (this.id === 'mic-toolbox-content-images') {
        boost_magnific_AccessTool.prototype.imagesChange();
    }
...
boost_magnific_AccessTool.prototype.keyboardRootEnable = function () {
    if (window.boost_magnific_toolboxAppstate.keyboardRoot) {
...
// FONTS CHANGE
boost_magnific_AccessTool.prototype.fontsChange = function (event) {
    event.preventDefault();

    // var mainBody = Number(document.body.style.fontSize.split('px')[0]);

    var counter = window.boost_magnific_toolboxAppstate.fontSize;
    ...
    if (this.id === 'mic-toolbox-fonts-down') {
        if (counter <= 1) {
            window.boost_magnific_toolboxAppstate.fontSize = 1;
            boost_magnific_AccessTool.prototype.updateState();
            return;
        }
        var items = document.querySelectorAll('body,h1,h2,h3,h4,h5,h6,p,a,button,input,textarea,li,td,th,strong,span,blockquote,div');
    ...
    window.boost_magnific_toolboxAppstate.fontSize = counter;
    boost_magnific_AccessTool.prototype.getFontsChanges(counter);
    boost_magnific_AccessTool.prototype.updateState();
};

// INITIAL FONT SIZE
boost_magnific_AccessTool.prototype.initFontsChange = function () {
    var items = document.querySelectorAll('body,h1,h2,h3,h4,h5,h6,p,a,button,input,textarea,li,td,th,strong,span,blockquote,div');
    var initFontSize = window.boost_magnific_toolboxAppstate.fontSize;
    for (var i = 0; i < items.length; i++) {
        var item = items[i];
...
boost_magnific_AccessTool.prototype.initFontsChangeFirst = function () {
    var items = document.querySelectorAll('body,h1,h2,h3,h4,h5,h6,p,a,button,input,textarea,li,td,th,strong,span,blockquote,div');
    for (var i = 0; i < items.length; i++) {
        var item = items[i];
        var font = window.getComputedStyle(item).getPropertyValue('font-size');
...
boost_magnific_AccessTool.prototype.getFontsChanges = function (initFontSize) {
    if (initFontSize > 1) {
...
// IMAGES CHANGE
boost_magnific_AccessTool.prototype.imagesChange = function () {
    ...
    if (document.body.classList.contains('mic-toolbox-content-images')) {
        ...
        window.boost_magnific_toolboxAppstate.imagesTitle = false;
    } else {
        this.imagesAddTitles();
        window.boost_magnific_toolboxAppstate.imagesTitle = true;
    }
};

boost_magnific_AccessTool.prototype.imagesAddTitles = function () {
...
boost_magnific_AccessTool.prototype.updateState = function () {
    var jsonSting = JSON.stringify(window.boost_magnific_toolboxAppstate);
    if (typeof (Storage) !== "undefined") {
        localStorage.setItem('boost_magnific_ACCESSTOOL', jsonSting);
    } else {
        console.log('No Storage Found');
    }
};

boost_magnific_AccessTool.prototype.openBox = function (event) {
    this.toolBox.classList.add('opened-mic-access-tool');
    if (!window.boost_magnific_toolboxAppstate.initFontSize || window.boost_magnific_toolboxAppstate.fontSize <= 1) {
        this.initFontsChangeFirst();
        window.boost_magnific_toolboxAppstate.initFontSize = true;
    }
    this.toolBoxCloseButton.focus();
};

boost_magnific_AccessTool.prototype.closeBox = function (event) {
    this.toolBox.classList.remove('opened-mic-access-tool');
};

boost_magnific_AccessTool.prototype.openCloseBoxKeyboard = function (event) {
    if (event.keyCode == 27) {
        this.closeBox();
    }
...
boost_magnific_AccessTool.prototype.resetApp = function (event) {
    localStorage.removeItem('boost_magnific_ACCESSTOOL');
    window.location.reload();
};
