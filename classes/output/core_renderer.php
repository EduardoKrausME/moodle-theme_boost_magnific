...
class core_renderer extends \core_renderer {
    ...
    public function get_course_image() {
        ...
        $bannerfileurlcourse = theme_boost_magnific_setting_file_url("banner_course_file_{$course->id}");
    ...
    public function standard_head_html() {
        ...
        if (!empty($this->page->course->id) && $this->page->course->id > 1) {
            $coursecolor = get_config("theme_boost_magnific", "override_course_primarycolor_{$this->page->course->id}");
        }
        if (!$coursecolor) {
