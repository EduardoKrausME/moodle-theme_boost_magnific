...
class core_renderer extends \core_renderer {
    ...
    public function full_header() {
        ...
        if ($hasuri) {
            ...
            $showcoursesummarycourse = get_config("theme_boost_magnific", "course_summary_banner_{$courseid}");
            ...
            if ($this->page->user_is_editing() && has_capability("moodle/site:config", $this->page->context)) {
                $url = "{$CFG->wwwroot}/theme/boost_magnific/quickstart/course-banner.php?courseid={$courseid}";
    ...
    private function get_details() {
        ...
        $details[] = [
            ...
            "text" => get_string("details-users", "theme_boost_magnific"),
        ];
        ...
        if (count($teachers)) {
            ...
            $details[] = [
                ...
                "text" => get_string("details-teachers", "theme_boost_magnific"),
            ];
        }
        ...
        $details[] = [
            ...
            "text" => get_string("details-emprogresso", "theme_boost_magnific"),
        ];
        $details[] = [
            "id" => "completaram",
            ...
            "text" => get_string("details-completaram", "theme_boost_magnific"),
        ];
        ...
        $details[] = [
            ...
            "link" => false,
            "number" => number_format($total, 0, $decsep, $thousandssep),
            "text" => get_string("details-not-access", "theme_boost_magnific"),
        ];
    ...
    public function get_default_image_for_courseid($courseid): string {
        global $CFG;
        return "{$CFG->wwwroot}/theme/boost_magnific/course-image-default.php?id={$courseid}";
    }
    ...
    public function brandcolor_background_menu_class() {
        ...
        if (get_config("theme_boost_magnific", "top_scroll_fix")) {
    ...
    public function render_login(login $form) {
        ...
        return $this->render_from_template("theme_boost_magnific/core/theme_login_form", $mustachecontext);
    }
    ...
    private function get_login_logo_url($maxwidth = 250, $maxheight = 130) {
        ...
        $logologin = get_config("theme_boost_magnific", "logologin");
        if (empty($logologin)) {
            return false;
        }
    ...
    public function standard_head_html() {
        ...
        foreach ($urls as $url) {
            ...
            if (!empty($this->page->course) && !empty($this->page->course->id) && $this->page->course->id > 1) {
                $coursecolor = (string)get_config("theme_boost_magnific", "override_course_color_{$this->page->course->id}");
            }

            // Check profile.
            $profileid = 0;
            $plugins = get_plugins_with_function("theme_boost_magnific_get_active_profile_id");
            foreach ($plugins as $plugintype => $names) {
                foreach ($names as $pluginname => $functionname) {
            ...
            if (isset($coursecolor[3]) || $profileid > 0) {
                ...
                $newurl = preg_replace_callback(
                    '/boost_magnific\/(\d+)(?:_\d+)?\//',
                    function ($matches) {
                        $novoid = ((int) $matches[1]);

                        return "boost_magnific/{$novoid}_{$this->page->course->id}/";
                    },
                    $newurl
                );
                $newurl = str_replace("theme/styles.php", "theme/boost_magnific/theme-styles.php", $newurl);
                $courseid = $this->page->course->id;
            } else {
                $newurl = $url->out(false);
                $courseid = 0;
