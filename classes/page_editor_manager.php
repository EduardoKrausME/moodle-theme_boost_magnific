...
/**
 * Helper for linking mod_page content to records stored in theme_boost_magnific_pages.
 */
class page_editor_manager {

    /** @var string Local prefix used in theme_boost_magnific_pages.local. */
    ...
    private const REQUEST_FIELD = 'theme_boost_magnific_use_editor';
    ...
    public static function get_by_cmid(int $cmid) {
        ...
        $records = $DB->get_records('theme_boost_magnific_pages', $conditions, 'sort ASC, id ASC', '*', 0, 1);
        if ($records) {
    ...
    public static function get_builder_url(int $cmid): moodle_url {
        return new moodle_url('/theme/boost_magnific/_editor/page-builder.php', ['cmid' => $cmid]);
    }
    ...
    public static function get_editor_url(stdClass $editorpage): moodle_url {
        ...
        return new moodle_url('/theme/boost_magnific/_editor/editor.php', $params);
    }
    ...
    public static function create_or_update_for_cmid(int $cmid): ?stdClass {
        ...
        if (!$editorpage) {
            require_once($CFG->dirroot . '/theme/boost_magnific/_editor/editor-lib.php');

            $lang = $USER->lang ?? $CFG->lang;
            $editorpage = theme_boost_magnific_editor_create_page(self::DEFAULT_TEMPLATE, $lang, self::local_from_cmid($cmid));
        ...
        $DB->update_record('theme_boost_magnific_pages', $editorpage);
    ...
    public static function sync_cmid_to_mod_page(int $cmid): void {
        ...
        $records = $DB->get_records('theme_boost_magnific_pages', ['local' => $local], 'sort ASC, id ASC');

        if (!$records) {
            $modulepage->content = '<p></p>';
        } else {
            $compiled = theme_boost_magnific_compile_pages($records, $lang, false);
            $content = '';
    ...
    public static function refresh_from_mod_page(int $cmid): void {
        ...
        $records = $DB->get_records('theme_boost_magnific_pages', ['local' => self::local_from_cmid($cmid)], 'sort ASC, id ASC');
        if (!$records) {
            return;
        }
    ...
    public static function delete_for_cmid(int $cmid): void {
        ...
        $DB->delete_records('theme_boost_magnific_pages', ['local' => self::local_from_cmid($cmid)]);
        cache::make('theme_boost_magnific', 'frontpage_cache')->purge();
    }
