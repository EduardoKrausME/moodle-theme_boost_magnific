...
class hook_callbacks {
    ...
    private static function mod_page_editor_integration(before_footer_html_generation $hook): void {
        ...
        if ($config) {
            ...
            $PAGE->requires->js_call_amd('theme_boost_magnific/pageeditor', 'init', [$config]);
        }
        ...
        $label = get_string('pageeditor_open_floating', 'theme_boost_magnific');
        $jslabel = json_encode(get_string('pageeditor_open_floating', 'theme_boost_magnific'));
        $title = get_string('pageeditor_title', 'theme_boost_magnific');

        $hook->add_html(
            <<<HTML
<style>
.theme-boost_magnific-pageeditor-floating {
    position: fixed;
    right: 1.25rem;
    bottom: 1.25rem;
    z-index: 1040;
...
.theme-boost_magnific-pageeditor-inline {
    margin-bottom: 1rem;
}
</style>
<script>
document.addEventListener('DOMContentLoaded', function() {
    ...
    button.className = 'btn btn-primary theme-boost_magnific-pageeditor-inline';
    button.href = href;
...
<a class="btn btn-primary theme-boost_magnific-pageeditor-floating" href="{$viewconfig['editorurl']}" aria-label="{$title}">{$label}</a>
HTML
        );
    }
    ...
    public static function before_http_headers(before_http_headers $hooks) {
        ...
        if (!empty($USER->theme_boost_magnific_redirect_to_editor)) {
            $redirecturl = $USER->theme_boost_magnific_redirect_to_editor;
            unset($USER->theme_boost_magnific_redirect_to_editor);
            if (is_numeric($redirecturl)) {
                redirect(new moodle_url("/theme/boost_magnific/_editor/editor.php", ["dataid" => $redirecturl]));
            }
