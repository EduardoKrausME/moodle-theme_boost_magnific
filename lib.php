...
/**
 * Function theme_boost_magnific_color
 *
 * @param $colorname
 *
 * @return string
 ...
 */
function theme_boost_magnific_color($colorname) {
...
function theme_boost_magnific_serve_css($filename) {
    ...
    if (!empty($CFG->themedir)) {
        $thestylepath = $CFG->themedir . "/boost_magnific/style/";
    } else {
        $thestylepath = $CFG->dirroot . "/theme/boost_magnific/style/";
    }
    ...
    if ((($ifmodifiedsince) && (strtotime($ifmodifiedsince) == $lastmodified)) || $etagheader == $etagfile) {
        theme_boost_magnific_send_unmodified($lastmodified, $etagfile);
    }
    theme_boost_magnific_send_cached_css($thestylepath, $filename, $lastmodified, $etagfile);
}
...
function theme_boost_magnific_get_logo($local = null) {
    ...
    $callbacks = get_plugins_with_function("theme_boost_magnific_get_logo");
    ...
    $cache = \cache::make("theme_boost_magnific", "logo_cache");
    $cachekey = "theme_boost_magnific_get_logo";
...
/**
 * theme_boost_magnific_get_body_class
 *
 * @return string
 * @throws coding_exception
 */
function theme_boost_magnific_get_body_class() {
    $color = theme_boost_magnific_get_setting("background_color", false);
    $color = str_replace("#", "", $color);
    return "boost_magnific-theme-{$color}";
}
 ...
 * @throws coding_exception
 */
function theme_boost_magnific_get_setting($setting, $format = true) {
    global $CFG;
...
/**
 * theme_boost_magnific_setting_file_url
 *
 * @param string $setting
 ...
 * @return string
 */
function theme_boost_magnific_theme_url() {
    global $CFG, $PAGE;
 ...
 * @return string
 */
function theme_boost_magnific_strip_html_tags($text) {
    $text = preg_replace(
        [
 ...
 * @return string $out
 */
function theme_boost_magnific_course_trim_char($str, $n = 500, $endchar = "&#8230;") {
    if (strlen($str) < $n) {
        return $str;
    }
 ...
 * @return string
 */
function theme_boost_magnific_get_hexa($hexa, $opacity) {
    if (!empty($hexa)) {
...
function theme_boost_magnific_coursemodule_edit_post_actions($data, $course) {
    ...
    if (isset($data->theme_boost_magnific_customicon)) {
        ...
        $filesave = file_save_draft_area_files(
            ...
            $context->id,
            "theme_boost_magnific", "theme_boost_magnific_customicon", $data->coursemodule,
            $options);
...
function theme_boost_magnific_process_css($css, $theme) {
    ...
    // Local css.
    if (@file_exists(__DIR__ . "/style/boost_magnific.css")) {
        $css .= file_get_contents(__DIR__ . "/style/boost_magnific.css");
    }
    ...
    $secondary = theme_boost_magnific_color("theme_color__color_secondary");
    $buttons = theme_boost_magnific_color("theme_color__color_buttons");

    // Fonts.
    $fontfamilytext = theme_boost_magnific_get_setting("fontfamily");
    $fontfamilytext = isset($fontfamilytext[3]) ? "{$fontfamilytext}," : "";

    $fontfamilytitle = theme_boost_magnific_get_setting("fontfamily_title");
    $fontfamilytitle = isset($fontfamilytitle[3]) ? "{$fontfamilytitle}," : "";

    $fontfamilysitename = theme_boost_magnific_get_setting("fontfamily_sitename");
    $fontfamilysitename = isset($fontfamilysitename[3]) ? "{$fontfamilysitename}," : "";

    $fontfamilymenus = theme_boost_magnific_get_setting("fontfamily_menus");
    $fontfamilymenus = isset($fontfamilymenus[3]) ? "{$fontfamilymenus}," : "";

    $textcolor = theme_boost_magnific_get_setting("background_text_color", false);
...
/**
 * Function theme_boost_magnific_before_footer
 *
 * @throws dml_exception
 */
function theme_boost_magnific_before_footer() {
...
/**
 * Function theme_boost_magnific_add_htmlattributes
 *
 * @return array
 *
 * @throws coding_exception
 */
function theme_boost_magnific_add_htmlattributes() {
    return \theme_boost_magnific\core_hook_output::html_attributes();
}
