...
$builderurl = new moodle_url('/theme/boost_magnific/_editor/page-builder.php', ['cmid' => $cmid]);
...
$compiledpages = theme_boost_magnific_compile_pages($records, $lang, true);
...
foreach ($compiledpages->pages as $blockpage) {
    ...
    $blockpage->deleteurl = (new moodle_url('/theme/boost_magnific/_editor/editor.php', [
        'dataid' => $blockpage->id,
        'delete' => 1,
        'returnurl' => $returnurl,
    ]))->out(false);
...
$templatecontext = [
    'title' => get_string('pageeditor_builder_title', 'theme_boost_magnific'),
    'description' => get_string('pageeditor_builder_description', 'theme_boost_magnific'),
...
$PAGE->requires->strings_for_js(['preview', 'add_block_edit'], 'theme_boost_magnific');
$PAGE->requires->js_call_amd('theme_boost_magnific/frontpage', 'add_block', [[
    'lang' => $lang,
    'local' => $local,
    'returnurl' => $returnurl,
]]);
$PAGE->requires->js_call_amd('theme_boost_magnific/frontpage', 'block_order', [$local]);

echo $OUTPUT->header();
echo $OUTPUT->render_from_template('theme_boost_magnific/page_builder', $templatecontext);
echo $OUTPUT->footer();
